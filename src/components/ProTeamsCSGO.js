...
class ProTeamsCSGO extends Component {
    state = {
        lolteams: [],
        lolplayers: []
      };
      ...
      componentDidMount() {
        ...
        axios.get(`https://api.pandascore.co/csgo/teams?token=${API_KEY}`).then(res => {
          console.log(res);
          const lolteams = res.data;
          this.setState({ lolteams });
        });
        axios.get(`https://api.pandascore.co/csgo/players?token=${API_KEY}`).then(res => {
            console.log(res);
    ...
    render() {
        return (
            <div style={{marginTop: '140px'}}>
                <h1 style={{textAlign: 'center', marginTop: '200px'}}>CSGO Pro Teams</h1>
                <div>
                ...
                <div>
                <div>
                    <h1 style={{textAlign: 'center', marginTop: '100px'}}>CSGO Pro Players</h1>
                </div>
                {this.state.lolplayers.map(player  => (
                    ...
                    {player.image_url ? 
                        ...
                        <div>{player.name}</div>
                        <div style={{color: '#09e6bc'}}>{'(CSGO)'}</div>
